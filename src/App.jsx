...
function Tbl({headers=[],rows=[],empty="Belum ada data",widths}){var C=useTheme();var fixed=!!widths;return <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:13.5,tableLayout:fixed?"fixed":"auto"}}>{fixed&&<colgroup>{headers.map((h,i)=><col key={i} style={{width:widths[i]?widths[i]+"px":"auto"}}/>)}</colgroup>}<thead><tr>{headers.map((h,i)=><th key={i} style={{padding:"9px 11px",background:C.nav,color:C.gl2,fontWeight:700,textAlign:"left",fontSize:11.5,borderBottom:"2px solid "+C.bdr,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{h}</th>)}</tr></thead><tbody>{!rows.length?<tr><td colSpan={headers.length} style={{padding:20,textAlign:"center",color:C.gl2}}>{empty}</td></tr>:rows.map((r,i)=><tr key={i} style={{borderBottom:"1px solid "+C.bdr}}>{r.map((c,j)=><td key={j} style={{padding:"8px 11px",color:C.wht,verticalAlign:"middle",overflow:"hidden"}}>{c}</td>)}</tr>)}</tbody></table></div>;}
...
<colgroup>{columns.map(c=><col key={c.key} style={{width:c.width?(typeof c.width==="number"?c.width+"px":c.width):"auto"}}/>)}</colgroup>
<thead>
...
<div style={{maxWidth:mobile?980:1400,margin:"0 auto"}}>{renderContent()}</div>
</div>
</div>
